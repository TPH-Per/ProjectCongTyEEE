  ...
  <transition name="slide-up">
    <div v-if="cartItemCount > 0" class="bottom-cart-bar">
      <div class="cart-info">
        ...
        <div class="cart-text">
          <h4 class="cart-title">{{ $t('customer.bottomCartBar.title') }}</h4>
          <p class="cart-summary">
            {{ $t('customer.bottomCartBar.selected') }} <span class="highlight">{{ $t('customer.bottomCartBar.items', { count: cartItemCount }) }}</span> · {{ $t('customer.bottomCartBar.total') }} <span class="highlight-price">{{ cartTotalDisplay }}</span>
          </p>
        </div>
      </div>

      <button @click="emit('go-to-cart')" class="btn-go-cart">
        {{ $t('customer.bottomCartBar.viewCart') }}

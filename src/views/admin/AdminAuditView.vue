...
const fetchLogs = async () => {
  ...
  if (error) {
    ...
    const [branchesRes, usersRes] = await Promise.all([
      branchIds.length > 0 ? supabase.from('branches').select('branch_id, name').in('branch_id', branchIds) : Promise.resolve({ data: [] }),
      userIds.length > 0 ? supabase.from('users').select('id, full_name').in('id', userIds) : Promise.resolve({ data: [] })
    ]);

    const branchMap = Object.fromEntries((branchesRes.data || []).map((b: any) => [b.branch_id, { ...b, id: b.branch_id }]));

-- 1. Modify Suppliers Table
ALTER TABLE public.suppliers
  ADD COLUMN IF NOT EXISTS contract_type TEXT DEFAULT 'NON_CONTRACTED',
  ADD COLUMN IF NOT EXISTS payment_due_day INTEGER CHECK (payment_due_day >= 1 AND payment_due_day <= 31);

-- 2. Modify Requisitions Table
ALTER TABLE public.requisitions
  ADD COLUMN IF NOT EXISTS items_jsonb JSONB DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS quotes_jsonb JSONB DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS selected_quote_id TEXT;

-- 3. Function to Approve Requisition and Create Goods Receipt
...
BEGIN
  -- Validate Role
  IF NOT public.has_role(ARRAY['admin', 'procurement']::user_role[]) THEN
  ...
  INSERT INTO public.goods_receipts (
    id, branch_id, receipt_number, supplier_id, status, notes, total_amount, payment_status
  ) VALUES (
    v_receipt_id,
    v_req.branch_id,
  ...
  IF jsonb_typeof(v_req.items_jsonb) = 'array' AND jsonb_array_length(v_req.items_jsonb) > 0 THEN
    ...
    LOOP
      INSERT INTO public.goods_receipt_items (
        receipt_id, ingredient_id, quantity_received, unit_price
      ) VALUES (
        v_receipt_id,
  ...
  SET total_amount = (
    SELECT COALESCE(SUM(quantity_received * unit_price), 0)

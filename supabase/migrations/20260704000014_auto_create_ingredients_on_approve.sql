-- Update approve_requisition_and_receipt to auto-create ingredients by name
...
BEGIN
  ...
  IF jsonb_typeof(v_req.items_jsonb) = 'array' AND jsonb_array_length(v_req.items_jsonb) > 0 THEN
    ...
    LOOP
      ...
      WHERE name = v_ing_name
      LIMIT 1;

      -- Create if not exists
      IF v_ing_id IS NULL THEN
        v_ing_id := gen_random_uuid();
        INSERT INTO public.ingredients (id, name, unit, category, is_active)
        VALUES (v_ing_id, v_ing_name, 'unit', 'General', true);
      END IF;
